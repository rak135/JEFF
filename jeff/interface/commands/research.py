...
from .models import CommandResult, InterfaceContext, ResearchCommandSpec
from .support.research_support import (
...
from .support.scope_resolution import get_work_unit, require_scoped_project, resolve_or_create_active_run
from .support.transitions import ensure_project_exists, ensure_work_unit_exists
...
def maybe_handoff_research_record_to_memory(*, context: InterfaceContext, spec: ResearchCommandSpec, record: ResearchArtifactRecord):
    if not spec.handoff_memory:
        return None
    memory_store = require_memory_store(context)
    return handoff_persisted_research_record_to_memory(artifact_record=record, memory_store=memory_store)


def run_research_backend(*, spec: ResearchCommandSpec, research_request: ResearchRequest, context: InterfaceContext, debug_emitter=None):
...
def annotate_research_runtime_error(exc, *, research_request: ResearchRequest, spec: ResearchCommandSpec, debug_events):
    setattr(exc, "question", research_request.question)
    setattr(exc, "provided_input_count", len(spec.inputs))
    setattr(exc, "stage", "synthesis")
    checkpoint = latest_research_checkpoint(debug_events)
    if checkpoint is not None:
        setattr(exc, "checkpoint", checkpoint)
    ...
    if resolved_source_count is not None:
        setattr(exc, "resolved_source_count", resolved_source_count)


def research_surface_error_from_backend_failure(*, spec: ResearchCommandSpec, research_request: ResearchRequest, exc: Exception, debug_events):
    reason = str(exc)
...
def proposal_input_package_from_research_artifact(*, record: ResearchArtifactRecord, research_artifact: ResearchArtifact) -> ProposalInputPackage:
